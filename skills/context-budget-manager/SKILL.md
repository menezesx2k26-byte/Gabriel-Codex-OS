...
3. Read/search only those inputs.
4. Form a concrete implementation hypothesis.
5. Inspect additional context only to resolve a named uncertainty or validate the hypothesis.
6. Execute the smallest coherent change.
7. Run proportionate validation.
8. Persist durable facts needed by future sessions, then allow transient exploration details to fall out of context.
